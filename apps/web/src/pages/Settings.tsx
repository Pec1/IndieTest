  ...
  return (
    <div className="space-y-2">
      ...
      <div className="relative group flex items-center border border-zinc-700 bg-zinc-900/30 transition-colors">
        <div className="pl-3 pr-2 text-zinc-500"><Icon size={16} strokeWidth={1.5} /></div>
        <input type="text" value={value} readOnly className="w-full bg-transparent text-zinc-500 rounded-none py-3 px-2 font-mono text-sm outline-none cursor-not-allowed" />
  ...
  return (
    <div className="flex items-start justify-between gap-4 p-4 bg-[#0F1013] border border-[#2C2D35] hover:border-[#4A3AFF]/30 transition-colors group">
      <div className="flex-1">
        <div className="font-display font-bold text-white uppercase tracking-wide text-sm mb-1">{label}</div>
        <p className="font-mono text-[10px] text-zinc-500 uppercase leading-relaxed">{description}</p>
      </div>
      <div className="flex-shrink-0 flex flex-col items-end gap-2">
        <button type="button" onClick={() => onChange(!enabled)}
          className={cn("w-12 h-6 border-2 transition-colors relative", enabled ? "bg-[#D4FF00] border-[#D4FF00]" : "bg-[#0F1013] border-[#2C2D35]")}>
          <div className={cn("absolute top-0.5 w-4 h-4 bg-black transition-all", enabled ? "left-[calc(100%-1.125rem)]" : "left-0.5")} />
        </button>
        <span className={cn("font-mono text-[10px] font-bold uppercase tracking-widest", enabled ? "text-[#D4FF00]" : "text-zinc-600")}>
    ...
    <div className="min-h-screen bg-[#0F1013] text-white selection:bg-[#D4FF00] selection:text-black flex flex-col">
      ...
      <main className="flex-1 w-full max-w-[1200px] mx-auto p-4 md:p-8">
        <div className="mb-8">
          ...
          <p className="font-mono text-xs text-zinc-500 uppercase">Gerenciamento de Perfil e Preferências de Segurança</p>
        </div>
          ...
          <div className={cn("relative overflow-hidden border-2 p-8", isDev ? "border-[#D4FF00] bg-[#D4FF00]/5" : "border-[#4A3AFF] bg-[#4A3AFF]/5")}>
            ...
            <div className="relative z-10 flex flex-col md:flex-row items-center gap-6">
              ...
              <div className="flex-1 text-center md:text-left">
                <span className="font-mono text-[10px] text-zinc-500 uppercase tracking-widest block mb-2">NÍVEL_DE_ACESSO // AUTENTICADO</span>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
          <div className="space-y-8">
            <section className="bg-[#1C1D22] border border-[#2C2D35] p-6">
              ...
              <div className="space-y-4">
                ...
                <div className="flex items-start gap-2 bg-[#0F1013] border border-[#2C2D35] p-3 mt-4">
              ...
              <form onSubmit={handlePasswordChange} className="space-y-4">
                  ...
                  return (
                    <div key={label} className="space-y-2">
                      ...
                      <div className="relative group flex items-center border border-[#2C2D35] bg-[#0F1013] focus-within:border-[#D4FF00] transition-colors">
                        <div className="pl-3 pr-2 text-zinc-500"><Lock size={16} strokeWidth={1.5} /></div>
                ...
                <div className="mt-6 pt-4 border-t border-[#2C2D35]">
                  <div className="flex items-start gap-2">
                    <CheckCircle2 size={14} className="text-[#D4FF00] mt-0.5 flex-shrink-0" />
                    <p className="font-mono text-[10px] text-zinc-500 leading-relaxed">Autenticação baseada em tokens JWT e criptografia ativa via algoritmo bcrypt.</p>
                  </div>
                </div>
              </form>
            </section>
          ...
          <div className="space-y-8">
            <section className="bg-[#1C1D22] border border-[#2C2D35] p-6">
              ...
              <div className="mt-6 pt-4 border-t border-[#2C2D35]">
                <button type="button" className="w-full font-display font-bold uppercase tracking-widest px-6 py-3 bg-[#1C1D22] text-white border border-[#2C2D35] hover:bg-[#25262c] hover:border-[#4A3AFF] transition-colors">
                  Salvar Preferências
                </button>
              </div>
            </section>
            <section className="bg-[#1C1D22] border-2 border-red-500/30 p-6 relative overflow-hidden">
              ...
              <div className="relative z-10">
                <div className="flex items-center gap-2 mb-4">
                  ...
                  <h3 className="font-display font-black text-xl text-white uppercase tracking-tight">Zona de Perigo</h3>
                </div>
                <p className="font-mono text-xs text-zinc-400 mb-6 uppercase leading-relaxed">Ações irreversíveis que afetam permanentemente sua conta e dados associados.</p>
