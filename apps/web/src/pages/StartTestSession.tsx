...
import { Crosshair, ArrowLeft, Play, Monitor, Cpu, CheckCircle2, AlertTriangle } from 'lucide-react';
    ...
    <div className="min-h-screen bg-[#0F1013] text-white selection:bg-[#D4FF00] selection:text-black flex flex-col">
      ...
      <main className="flex-1 w-full max-w-4xl mx-auto p-4 md:p-8">
        <div className="mb-8">
          ...
          <p className="font-mono text-xs text-zinc-500 mt-2">PROTOCOLO DE INICIALIZAÇÃO DE AMBIENTE DE TESTES BETA.</p>
        </div>
        {sucesso ? (
          <div className="bg-[#1C1D22] border-2 border-[#D4FF00] p-12 flex flex-col items-center justify-center gap-6 text-center">
            <CheckCircle2 size={64} className="text-[#D4FF00]" />
            <div>
              <h2 className="font-display font-black text-3xl text-white uppercase mb-2">SESSÃO_INICIADA</h2>
              <p className="font-mono text-sm text-zinc-400">{sucesso}</p>
              <p className="font-mono text-xs text-zinc-500 mt-2">Redirecionando para reportar bugs...</p>
            </div>
          </div>
        ) : (
          <form onSubmit={handleSubmit} className="space-y-0">
            ...
            <div className="bg-[#1C1D22] border border-[#2C2D35] p-6 sm:p-8">
                ...
                <div className="space-y-2">
                  ...
                  <div className="relative group flex items-center border border-[#2C2D35] bg-[#0F1013] focus-within:border-[#D4FF00] transition-colors">
                    ...
                    <select value={projetoId} onChange={e => setProjetoId(e.target.value)} required
                      ...
                      {projetos.map(p => <option key={p.id} value={p.id} className="bg-[#1C1D22]">{p.nome} — {p.desenvolvedor.nomeEstudio}</option>)}
                    </select>
                  </div>
                ...
                <div className="space-y-2">
                  ...
                  <div className="relative group flex items-center border border-[#2C2D35] bg-[#0F1013] focus-within:border-[#D4FF00] transition-colors">
                    ...
                    <select value={versaoId} onChange={e => setVersaoId(e.target.value)} required disabled={!projetoId || versoes.length === 0}
                      ...
                      {versoes.map(v => <option key={v.id} value={v.id} className="bg-[#1C1D22]">{v.numeroVersao} — {v.status}</option>)}
                    </select>
                  </div>
              ...
              {versaoSelecionada && (
                <div className="mt-4 bg-[#0F1013] border border-[#D4FF00] p-4">
                  <div className="font-mono text-[10px] text-[#D4FF00] uppercase mb-2">CHANGELOG_DA_VERSÃO_SELECIONADA</div>
                  <pre className="font-mono text-xs text-zinc-300 leading-relaxed whitespace-pre-wrap">{versaoSelecionada.changelog}</pre>
                </div>
              )}
            </div>
            <div className="bg-[#1C1D22] border-x border-[#2C2D35] border-b p-6 sm:p-8">
                ...
                <div className="space-y-2">
                  ...
                  <div className="relative group flex items-center border border-[#2C2D35] bg-[#0F1013] focus-within:border-[#D4FF00] transition-colors">
                    ...
                    <select value={dispositivo} onChange={e => setDispositivo(e.target.value)} required
                      ...
                      {DISPOSITIVOS.map(d => <option key={d} value={d} className="bg-[#1C1D22]">{d}</option>)}
                    </select>
                  </div>
                  {dispositivo === 'Outro' && (
                    <div className="relative group flex items-center border border-[#2C2D35] bg-[#0F1013] focus-within:border-[#D4FF00] transition-colors mt-2">
                      <div className="pl-3 pr-2 text-[#D4FF00] font-mono font-bold select-none">&gt;</div>
                ...
                <div className="space-y-2">
                  ...
                  <div className="relative group flex items-center border border-[#2C2D35] bg-[#0F1013] focus-within:border-[#D4FF00] transition-colors">
                    ...
                    <select value={sistemaOperacional} onChange={e => setSistemaOperacional(e.target.value)} required
                      ...
                      {SISTEMAS.map(s => <option key={s} value={s} className="bg-[#1C1D22]">{s}</option>)}
                    </select>
                  </div>
              ...
              <div className="mt-6 bg-[#0F1013] border border-[#2C2D35] p-4">
                <div className="font-mono text-[10px] text-zinc-500 uppercase mb-3">DETECÇÃO_AUTOMÁTICA</div>
                <div className="grid grid-cols-1 md:grid-cols-2 gap-3 font-mono text-xs">
                  <div className="flex items-center gap-2">
                    <Monitor size={14} className="text-[#D4FF00]" />
                    <span className="text-zinc-400">Resolução: {window.screen.width}x{window.screen.height}</span>
                  </div>
                  <div className="flex items-center gap-2">
                    <Cpu size={14} className="text-[#D4FF00]" />
                    <span className="text-zinc-400">Núcleos: {navigator.hardwareConcurrency || '—'}</span>
                  </div>
                </div>
              </div>
            </div>
            <div className="bg-[#0F1013] border border-t-0 border-[#2C2D35] p-6 flex flex-col gap-6">
              <div className="flex items-start gap-3 bg-[#1C1D22] border-l-2 border-[#D4FF00] p-4">
                ...
                <div>
                  <span className="font-display font-bold text-sm text-white tracking-wide uppercase">Atenção // Protocolo de Sessão</span>
